...
def make_pruning_summary():
    ...
    def classify_layers(model_name):
        ...
        for i, l in enumerate(layers):
            ...
            elif f_pct > 3 and s_pct > 2:
                categories[i] = "tradeoff"
            elif abs(s_pct) < 5 and abs(f_pct) < 5:
                categories[i] = "safe"
            else:
                categories[i] = "uncertain"
    ...
    ax.set_title("剪枝安全边界：哪些层可以安全移除？（Spike-F1 约束下）",
                 fontsize=13, fontweight="bold")

    legend_elements = [
        mpatches.Patch(color=colors["must_keep"],    label="必须保留（移除显著损害精度或F1）"),
        mpatches.Patch(color=colors["both_benefit"], label="移除后双指标改善"),
        mpatches.Patch(color=colors["tradeoff"],     label="Trade-off（尖峰有利但精度受损）"),
        mpatches.Patch(color=colors["safe"],         label="可安全移除（SMAPE+F1 <5%）"),
        mpatches.Patch(color=colors["uncertain"],    label="需进一步验证"),
    ]
    ax.legend(handles=legend_elements, loc="upper right", fontsize=9, framealpha=0.9)

    for model, d in pruning_classifications.items():
        y = y_positions[model]
        removable = sum(1 for cat in d["categories"].values() if cat in ("safe", "both_benefit"))
        pct = removable / d["total"] * 100
        ax.text(20.7, y, f"可安全移除: {removable}/{d['total']}层 ({pct:.0f}%)",
...
| 9 | **简化输出头** | N/A | **+51%** | **+0%** | TimesFM 简单头已足够；Chronos 依赖复杂头 |
| 10 | **RoPE 位置编码** | +31% | +40% | **+0%** | 模型依赖差异显著 |
...
| TimesFM-2.5 | **+0%（完全无效）** | 可能通过 PerDimScale + QK-Norm 隐式编码位置信息 |
...
## 6. 核心发现五：大量层冗余，尤其是 TimesFM-2.5

在 **Spike-F1 Δ < 5%** 约束下，各模型可安全移除的层：

| 模型 | 总层数 | 可移除层 | 移除比例 | 必须保留层 |
|---|---|---|---|---|
| Toto-2.0 | 6 | L1, L2, L3 | 50% | L0（尖峰+精度），L4（移除F1改善），L5（输出层） |
| Chronos-2 | 6 | L2, L3, L4, L5 | 67% | L0（隐性尖峰），L1（尖峰次关键） |
| TimesFM-2.5 | 20 | L4-L6, L8-L16, L18-L19 | **70%** | L0, L7, L17（三个关键锚点） |

**TimesFM 结论**：200M 参数的 20 层深度在电价预测任务上**严重过参数化**。
一个仅含 L0、L7、L17 + 必要过渡层的精简版（约 5-6 层）理论上可达到接近相同的性能。
